...
class Gruff::BarNStacked < Gruff::Base
...
protected
  ...
  def draw_bars
    ...
    @norm_data.each_with_index do |nd, index|
      next if index == 1 || index == 2

      col_space = @data_column_spacing * @graph_width
      @bar_spacing ||= 1 # space between the bars
    ...
    @norm_data.last(2).each_with_index do |data_row, row_index|
      data_row[DATA_VALUES_INDEX].each_with_index do |data_point, point_index|
        @d = @d.fill data_row[DATA_COLOR_INDEX]
